...
function startServer(port) {
    server = restify.createServer().use(restrictToLocalhost).use(restify.plugins.bodyParser({mapParams: true})).use(restify.plugins.queryParser({mapParams: true})).use(network.connect);
...
program.option('-p, --port <port>', 'The port to start server').parse(process.argv);
