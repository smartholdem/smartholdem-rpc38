...
function createBip38(req, res, next) {
    ...
    then(function(acc){
        var transaction = sthjs.transaction.createTransaction(req.params.recipientId, req.params.amount, null, "dummy");
        transaction.senderPublicKey = acc.keys.getPublicKeyBuffer().toString("hex");
        delete transaction.signature;
        sthjs.crypto.sign(transaction, acc.keys);
...
function create(req, res, next) {
    ...
    var transaction = sthjs.transaction.createTransaction(req.params.recipientId, amount, null, req.params.passphrase);
    leveldb.
    setObject(transaction.id, transaction).
    then(function(){
        res.send({
